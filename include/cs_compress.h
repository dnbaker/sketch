...
template<typename C>
std::vector<unsigned> top_indices_from_compressed(const C &in, size_t newdim, size_t olddim, const KWiseHasherSet<4> &hf, unsigned k) {
    if(newdim < in.size()) throw 1;
    const size_t ns = hf.size();
    schism::Schismatic<uint32_t> div(olddim);
    using FT = std::decay_t<decltype(*std::begin(in))>;
    ...
    for(size_t i = 0; i < newdim; ++i) {
        ...
        {
            pq.push(std::make_pair((tmp[ns >> 1] + tmp[(ns - 1) >> 1]) * .5, i));
            if(pq.size() > k) pq.pop();
        }
    }
    std::vector<std::pair<FT, unsigned>> ret;
    ret.reserve(k);
    for(unsigned i = 0; i < k; ret[i++] = pq.top(), pq.pop());
    return ret;
}
