...
template<typename C>
auto cs_compress(const C &in, size_t newdim, const KWiseHasherSet<4> &hf_) {
    if(newdim > in.size()) throw 1;
    const size_t ns = hf_.size();
...
template<typename C>
auto cs_decompress(const C &in, size_t newdim, size_t olddim, const KWiseHasherSet<4> &hf_) {
    if(newdim < in.size()) throw 1;
    const size_t ns = hf_.size();
    ...
    for(size_t i = 0; i < newdim; ++i) {
        std::vector<FT> tmp(hf_.size());
        tmpbuffer<float, 8> mem(hf_.size());
        auto tmp = mem.get();
        for(unsigned j = 0; j < ns; ++j) {
            auto hv = h(i, j);
            tmp[j] = in[div.mod(hv >> 1) + j * olddim] * (hv & 1 ? 1: -1);
        }
        std::sort(tmp, tmp + hf_.size());
