...
int main() {
    ...
    Timer zomg("");
    uint64_t zomgsum = 0, vtimesum = 0;
    enum {FIRST_LOOP_N = 1000};
    for(size_t i = 0; i < FIRST_LOOP_N; ++i) {
    ...
    vtimesum /= FIRST_LOOP_N;
    zomgsum /= FIRST_LOOP_N;
    ...
    zomgsum = 0;
    {
        ...
        std::fprintf(stderr, "vtime each: %zu. zomgtime: %zu. Bulk processing for vector %zu. Total score for vector approach: %zu. Total score for other approach: %zu\n",
                              size_t(vtimesum), size_t(zomgsum), single_flattentime, vtimesum * FIRST_LOOP_N + single_flattentime, zomg + t.report());
        t.restart();
