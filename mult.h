...
template<template<typename> typename Policy=SizePow2Policy, typename CountType=uint32_t>
struct SuperMinHash {
    const Policy<CountType> pol_;
    using BType = typename std::make_signed<CountType>::type;
    uint64_t a_, i_;
    uint32_t m_;
    std::vector<CountType, Allocator<CountType>> p_;
    std::vector<CountType, Allocator<CountType>> q_;
    std::vector<BType, Allocator<BType>>         b_;
    ...
    // TODO: pack this into Final{Div,}BBitMinHashers
};
