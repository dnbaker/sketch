...
def hash_ngrams(x, n=3):
    def hash_ngram(y):
        from functools import reduce
        return reduce(lambda x, y: x ^ y, map(hash, y))
    return np.array([hash_ngram(x[i:i + n]) for i in range(len(x) - n + 1)], dtype=np.uint64)
